...
describe('Leaderboard page', () => {
  ...
  it('orders teams by pingas (desc) and then name (asc)', async () => {
    ...
    expect(rows[0]).toHaveStyle('--row-meter-start: #b7791f');
    expect(rows[1]).toHaveStyle('--row-meter-start: #64748b');
    expect(rows[2]).toHaveStyle('--row-meter-start: #8f4e2c');
    expect(rows[3]).toHaveStyle('--row-meter-start: #047857');
    expect(rows[0]).toHaveStyle('--row-meter-end: #e0a72f');
    expect(rows[1]).toHaveStyle('--row-meter-end: #cbd5e1');
    expect(rows[2]).toHaveStyle('--row-meter-end: #c47a45');
    expect(rows[3]).toHaveStyle('--row-meter-end: #22c55e');
    expect(rows[0]).toHaveStyle('--row-meter-rail: rgba(224, 167, 47, 0.16)');
    expect(rows[1]).toHaveStyle('--row-meter-rail: rgba(100, 116, 139, 0.12)');
    expect(rows[2]).toHaveStyle('--row-meter-rail: rgba(196, 122, 69, 0.14)');
